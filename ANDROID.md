...
телефон, а не компьютер. Есть два варианта:

---

### ✅ Вариант A — Intiface Mobile на телефоне (рекомендуется)

**Самый простой способ — автономная работа без ПК:**

1. **Установи Intiface Central на телефон:**
   - [Google Play Store](https://play.google.com/store/apps/details?id=org.metafetish.intiface_central)
   - Или скачай APK с [GitHub Releases](https://github.com/intiface/intiface-central-android/releases)

2. **Запусти Intiface Central:**
   - Открой приложение
   - Нажми **"Start Server"**
   - Сервер запустится на `localhost:12345`

3. **Открой Pleasure Control:**
   - На вкладке **Devices** адрес уже будет `ws://localhost:12345` (автоматически на Android)
   - Нажми **"Connect to Intiface"**
   - Затем **"Scan Devices"**
   - Игрушки подключатся напрямую через Bluetooth телефона

**Плюсы:**
- ✅ Полная автономность — не нужен компьютер
- ✅ Прямое BLE-подключение с телефона
- ✅ Все устройства, поддерживаемые Intiface, работают
- ✅ Одна настройка — дальше просто запускаешь оба приложения

**Минусы:**
- Нужно два приложения (Intiface + Pleasure Control)
- Intiface нужно запускать перед использованием

---

### Вариант B — Intiface на ПК, телефон по Wi-Fi

Если хочешь управлять с телефона, но Intiface запущен на компьютере:
...
**Плюсы:**
- Централизованное управление с ПК
- Можно управлять с нескольких устройств

**Минусы:**
- Требуется ПК и Wi-Fi
- Нужно знать IP компьютера
